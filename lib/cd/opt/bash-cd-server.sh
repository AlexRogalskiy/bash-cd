...
handle() {
    ...
    if [[ "$in" == "POST /push"* ]]; then
        ...
        if [ -f "$rollback_file" ]; then
            ...
            highlight "[$(date)] CODEBASE CHANGE DETECTED: ${in}"
            changed=1
...
#highlight "[$(date)] BASH-CD SERVER LISTENING ON PORT: $CD_PORT"
...
highlight "[$(date)] BASH-CD SERVER WATCHING REMOTE GIT REPO"
while [ 1 ]; do
